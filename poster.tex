  ...
  \begin{block}{Normalizing Flows}
    ...
    \begin{equation}
      p(x) = r(z)\left|\det{\frac{\partial z}{\partial x}}\right|
        ...
        %   r(z) = p(x)\left|\det{\frac{dx}{dz}}\right|^{-1}
        %   = p(f(z))\left|\det{\frac{\partial f}{\partial z}}\right|^{-1}
    \end{equation}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=0.9\columnwidth]{assets/flow_model5}
      \caption{Using a Flow to generate data \(x'\). Image adapted from~\cite{weng2018flow}}
    \end{figure}
    %

    We can construct a normalizing flow by composing multiple invertible flows so that \(x\equiv [f_1\circ f_{2}\circ
    \cdots \circ f_{K}](z)\).
    %
    In practice, the function \(f\) is usually composed of multiple \emph{coupling layers}, which update an ``active''
    ...
    variables are updated during the course of the flow.
    %
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Trivializing Map}
    ...
    \begin{equation}
      ...
      % \langle \mathcal{O}\rangle = \frac{1}{\mathcal{Z}}\int dz \,\mathcal{O}(z) e^{-S_{\beta}(z)}
      \label{eq:expval}
    \end{equation}.
...
\begin{column}{\colwidth}
  ...
  \begin{block}{2D U(1) Gauge Theory}
    ...
    %
    \begin{equation}
      p(x)\propto e^{- S(x)}, \quad \text{where}\quad S(x) \equiv \sum_{P} 1 - \cos x_{P}
    \end{equation}
    %
    and
    %
